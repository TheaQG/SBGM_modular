...
def test_dataset(args):
    ...
    print(f'\nHR data: {hr_model} {hr_var}, [{hr_units}]')
    for cond, path in lr_cond_dirs_zarr.items():
        print(f'LR data: {lr_model} {cond}, [{lr_units[lr_conditions.index(cond)]}]')
    print('\n')

    # Sampling options and geo arrays (omitted here for brevity—assume they’re set as before)
    ...
    # Convert the DataLoader batch to a list of sample dictionaries.
    from torch.utils.data import DataLoader
    dataloader = DataLoader(dataset, batch_size=5, shuffle=True)
    sample_batch = next(iter(dataloader))
    # Instead of indexing blindly, check if each value is a tensor before indexing.
    sample_list = []
    batch_size = sample_batch[list(sample_batch.keys())[0]].size(0)
    for i in range(batch_size):
        ...
        for key, value in sample_batch.items():
            if isinstance(value, torch.Tensor):
                ...
                sample[key] = value  # If not a tensor, store as is.
    ...
    if args.save_figs:
        fig.savefig(PATH_SAVE + f"dataset_sample_{hr_var}.png", dpi=300, bbox_inches='tight')
    if args.show_figs:
        plt.show()
