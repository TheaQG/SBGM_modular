...
def create_train_valid_test_data_zarr(args):
    ...
    LR_PATH_ALL = LR_PATH + 'all/'
    HR_PATH_ALL = HR_PATH + 'all/'

    # Check if /train, /valid, /test directories exist. If not, create them
    if not os.path.exists(LR_PATH + 'train2'):
        os.makedirs(LR_PATH + 'train2')
    if not os.path.exists(LR_PATH + 'valid2'):
        os.makedirs(LR_PATH + 'valid2')
    if not os.path.exists(LR_PATH + 'test2'):
        os.makedirs(LR_PATH + 'test2')

    if not os.path.exists(HR_PATH + 'train2'):
        os.makedirs(HR_PATH + 'train2')
    if not os.path.exists(HR_PATH + 'valid2'):
        os.makedirs(HR_PATH + 'valid2')
    if not os.path.exists(HR_PATH + 'test2'):
        os.makedirs(HR_PATH + 'test2')

    # Empty the /train, /valid, /test directories
    print(f'\nEmptying LR and HR directories')
    for file in os.listdir(LR_PATH + 'train2'):
        os.remove(LR_PATH + 'train2/' + file)
    for file in os.listdir(LR_PATH + 'valid2'):
        os.remove(LR_PATH + 'valid2/' + file)
    for file in os.listdir(LR_PATH + 'test2'):
        os.remove(LR_PATH + 'test2/' + file)

    for file in os.listdir(HR_PATH + 'train2'):
        os.remove(HR_PATH + 'train2/' + file)
    for file in os.listdir(HR_PATH + 'valid2'):
        os.remove(HR_PATH + 'valid2/' + file)
    for file in os.listdir(HR_PATH + 'test2'):
        os.remove(HR_PATH + 'test2/' + file)
